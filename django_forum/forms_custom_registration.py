...
class CustomUserCreation(auth.forms.UserCreationForm):
    ...
    class Meta(auth.forms.UserCreationForm.Meta):
        fields = (
            *auth.forms.UserCreationForm.Meta.fields,
            "email",
            "captcha",
        )
    ...
    def clean_username(self) -> str:
        ...
        if fuzz.ratio(username, self["display_name"].value()) > 69:
            self.add_error(
                "username", "Error! your username is too similar to your display name"
            )
            self.valid = False
        return username
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.helper.form_action = reverse_lazy("django_forum:register")
