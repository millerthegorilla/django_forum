...
@pytest.fixture()
def subscribed_user(active_user, post):
    post.subscribed_users.add(active_user)
    return active_user
...
def user_is_subscribed(db, post, subscribed_user):
    try:
        post.subscribed_users.get(username=subscribed_user.username)
